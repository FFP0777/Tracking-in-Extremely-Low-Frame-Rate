...
def filter_boxes(boxes, scores, classes, good_classes, min_confid=0.5, mask=None):
    ...
    final_scores: list(float)
        The scores corresponding to the boxes that matched all criteria.
    final_classes: list(int)
        The classes corresponding to the boxes that matched all criteria.
    """
    good_boxes = []
    final_scores = []
    final_classes = []
    ...
    for bbox, score, cl in zip(boxes, scores, classes):
        if score < min_confid or cl not in good_classes:
            continue
        good_boxes.append([float(coord) for coord in bbox]) 
        final_scores.append(float(score)) 
        final_classes.append(int(cl)) 
        
    if mask is None:
        return good_boxes, final_scores, final_classes

    final_boxes = []
    final_scores_masked = []
    final_classes_masked = []
    
    for bbox, score, cl in zip(good_boxes, final_scores, final_classes):
        cx, cy = float(bbox[0]), float(bbox[1])
        if mask[cy, cx] > 0:
            final_boxes.append(bbox)
            final_scores_masked.append(score)
            final_classes_masked.append(cl)
    
    return final_boxes, final_scores_masked, final_classes_masked
...
def run_mot(cfg: CfgNode):
    ...
    for frame_num, frame in enumerate(video_in):
        ...
        boxes = [[round(float(x), 2), round(float(y), 2), round(float(w), 2), round(float(h), 2)] for x, y, w, h in boxes]
        scores = [round(float(score), 2) for score in scores]
        boxes, scores, classes = filter_boxes(boxes, scores, classes, cfg.MOT.TRACKED_CLASSES, MIN_CONFID, det_mask)

        boxes_tlwh = [[float(x - w / 2), float(y - h / 2), float(w), float(h)] for x, y, w, h in boxes]
        benchmark.register_call("detection filter")

        features = extractor(frame, boxes_tlwh)
        detections = [Detection(bbox, score, clname, feature) for bbox, score, clname, feature in zip(boxes_tlwh, scores, classes, features)]
        features = torch.tensor(features, dtype=torch.float32)  # Ensure features are float
        benchmark.register_call("reid")

        boxs = np.array([d.tlwh for d in detections], dtype=np.float32)  # Ensure boxs is float
        scores = np.array([d.confidence for d in detections], dtype=np.float32)  # Ensure scores are float
        ...
        # get attributes
        static_attribs = static_extractor(frame, boxs, features) if static_extractor else {}
        dynamic_attribs = dynamic_extractor(frame, boxs, features) if dynamic_extractor else {}
        ...
        log.debug(f"Frame {frame_num}: active_track_ids: {active_track_ids}, frame type: {type(frame)}, {frame.dtype}, {frame.shape} .")

        if cfg.MOT.ONLINE_VIDEO_OUTPUT:
            video_out.update(frame, active_track_ids, active_track_bboxes_tlwh, all_attribs_list)

        if cfg.MOT.SHOW:
            display.update(frame, active_track_ids, active_track_bboxes_tlwh, all_attribs_list)

        benchmark.register_call("displays")

        fps_counter.step()
        print("\rFrame: {}/{}, fps: {:.3f}".format(frame_num, video_frames, fps_counter.value()), end="")
    ...
    final_tracks = list(filter(lambda track: len(track.frames) >= cfg.MOT.MIN_FRAMES, final_tracks))
    ...
    if cfg.MOT.VIDEO_OUTPUT:
        annotate_video_with_tracklets(cfg.MOT.VIDEO, os.path.join(cfg.OUTPUT_DIR, f"{MOT_OUTPUT_NAME}.{VIDEO_EXT}"), final_tracks, cfg.FONT, cfg.FONTSIZE)
