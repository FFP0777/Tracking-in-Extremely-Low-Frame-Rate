...
class ByteTrackerIOU(TrackerBase):
    def __init__(self,
                 frame_rate=1,
                 track_conf_thresh=0.6,
                 new_track_conf_thresh=0.6,
                 track_match_thresh=0.68, #0.7
                 lost_track_keep_seconds=0,
                 zone_matcher: Union[ZoneMatcher, None] = None,
                 ):
        super().__init__(zone_matcher)
    ...
    # """Update the tracker with detections from a new frame."""
        ...
        detections = monkey_patch_detections(detections, static_attributes, dynamic_attributes)

        byte_input = np.zeros((len(detections), 5), np.float32)
        features = np.zeros((len(detections), 2048), np.float32)   
        for i, det in enumerate(detections):
            byte_input[i, :4] = det.to_tlbr()
            byte_input[i, 4] = det.confidence
            features[i, :] = det.feature   

        stracks = self._tracker.update(byte_input, features)
        ...
        for strack in stracks:
            ...
            if strack.last_det_idx is not None:
                det = detections[strack.last_det_idx]
                cx, cy = int(det.tlwh[0] + det.tlwh[2] / 2), int(det.tlwh[1] + det.tlwh[3] / 2)
                zone_id = self._zone_matcher.find_zone_for_point(cx, cy) if self._zone_matcher else None
                track.update(frame_num, det.tlwh, det.confidence, det.feature, det.static_attributes, det.dynamic_attributes, zone_id)
            else:
                log.warning(f"Track {strack.track_id} has no last detection index.")